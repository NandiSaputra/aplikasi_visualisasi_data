...
class MahasiswaBaru extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['mahasiswa'] = $this->MahasiswaBaru_model->getData();

		$this->load->view('template/admin/header.php');
		$this->load->view('template/admin/sidebar.php');
		$this->load->view('admin/mahasiswa_baru.php', $data);
...
$insertdata = $this->MahasiswaBaru_model->insert_batch($data);
if($insertdata){
	$this->session->set_flashdata('succes','sukses imposrt data');
	redirect(base_url('MahasiswaBaru/index'));
}else{
